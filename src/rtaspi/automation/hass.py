...
class HomeAssistant:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # API settings
        self.api_url = f"{'https' if self.use_ssl else 'http'}://{self.host}:{self.port}/api"
        self.ws_url = f"{'wss' if self.use_ssl else 'ws'}://{self.host}:{self.port}/api/websocket"
        self.headers = {
    ...
    def update_state(self, entity_id: str, state: Any, attributes: Optional[Dict] = None) -> bool:
        """Update entity state.
    ...
    def subscribe_events(self, event_type: str, callback: Callable[[Dict], None]) -> bool:
        ...
        try:
            ...
            def handle_event(msg):
                if msg.get('type') == 'event' and msg.get('event', {}).get('event_type') == event_type:
                    callback(msg['event'])
    ...
    def _send_ws_message(self, message: Dict, callback: Optional[Callable] = None) -> Optional[Dict]:
            ...
            if msg_type == 'auth_required':
                ...
                if msg_id in self._ws_callbacks:
                    ...
                    if not msg.get('success', True):
                        logger.error(f"Error in message {msg_id}: {msg.get('error', {}).get('message')}")
                    callback(msg)
