...
def check_gstreamer():
    ...
    try:
        result = subprocess.run(['gst-launch-1.0', '--version'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output = result.stdout.decode('utf-8')
        ...
        result = subprocess.run(['gst-inspect-1.0'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output = result.stdout.decode('utf-8')
...
def check_nginx():
    ...
    try:
        ...
        result = subprocess.run(['nginx', '-V'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output = result.stderr.decode('utf-8')  # nginx -V wypisuje konfigurację na stderr
