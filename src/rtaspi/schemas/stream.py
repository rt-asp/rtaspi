...
class StreamSource(BaseModel):
    ...
    stream_type: str = Field(
        ...,
        description="Type of stream (video, audio, or both)"
    )
    enabled: bool = Field(
        True,
...
class StreamFilter(BaseModel):
    ...
    order: int = Field(
        0,
        description="Order in which to apply the filter"
    )
...
class StreamOutput(BaseModel):
    ...
    name: str = Field(
        ...,
        description="Unique name for this output"
    )
    settings: Dict[str, Any] = Field(
        default_factory=dict,
        description="Output-specific settings"
    )
...
class StreamEncoding(BaseModel):
    ...
    video_codec: Optional[str] = Field(
        None,
        description="Video codec to use (e.g., H264, VP8)"
    )
    audio_codec: Optional[str] = Field(
        None,
        description="Audio codec to use (e.g., AAC, OPUS)"
    )
...
class StreamConfig(BaseModel):
    ...
    name: str = Field(
        ...,
        description="Unique name to identify the stream"
    )
    enabled: bool = Field(
        True,
    ...
    outputs: List[StreamOutput] = Field(
        default_factory=list,
        description="List of stream outputs"
    )
    encoding: Optional[StreamEncoding] = Field(
        None,
        description="Stream encoding configuration"
    )
    settings: Dict[str, Any] = Field(
        default_factory=dict,
        description="Stream-specific settings"
    )
...
class StreamStatus(BaseModel):
    ...
    error: Optional[str] = Field(
        None,
        description="Error message if stream is in error state"
    )
    start_time: Optional[str] = Field(
        None,
        description="ISO timestamp when stream started"
    )
    ...
    stats: Dict[str, Any] = Field(
        default_factory=dict,
        description="Stream-specific statistics"
    )
