...
class rtaspi:
    ...
    def __init__(self, config_path=None):
        ...
        if not dependencies.get('ffmpeg', {}).get('installed', False):
            logger.warning("FFmpeg nie jest zainstalowany, niektóre funkcje mogą nie działać")
    ...
    def start(self):
        ...
        try:
            ...
            network_devices_manager = NetworkDevicesManager(self.config, self.mcp_broker)
            ...
            self.mcp_client.publish("system/status", {
                "status": "started",
                "timestamp": time.time(),
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description='rtaspi - Real-Time Annotation and Stream Processing')
