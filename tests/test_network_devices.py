...
def test_network_manager_initialization(network_manager, test_config):
    ...
    assert network_manager.scan_interval == test_config['network_devices']['scan_interval']
...
def test_add_device_invalid_data(network_manager):
    ...
    with pytest.raises(ValueError):
        network_manager.add_device(
            name="Test Camera",
            ip="192.168.1.100"  # Missing required fields
        )
...
def test_update_device(network_manager):
    ...
    # Update device
    success = network_manager.update_device(
        device_id,
        name="Updated Camera",
        port=8554
    )
...
def test_discover_devices_with_errors(network_manager):
    ...
    network_manager.discovery_modules['onvif'].discover = MagicMock(side_effect=Exception("ONVIF Error"))
    network_manager.discovery_modules['upnp'].discover = MagicMock(return_value=[{
        'name': 'UPnP Camera',
        'ip': '192.168.1.102',
        'port': 80,
        'type': 'video',
...
def test_handle_command_invalid(network_manager):
    ...
    with pytest.raises(ValueError):
        network_manager._handle_command("command/network_devices/update", 
...
def test_save_and_load_state(network_manager, temp_dir):
    ...
    new_manager = NetworkDevicesManager(network_manager.config, network_manager.mcp_broker)
